...
def delete_instances():
    ...
    active_instances_dict = appengine.apps().services().versions()\
        .instances().list(servicesId=settings.SERVICE_ID,
                          appsId=settings.PROJECT_ID,
                          versionsId=settings.VERSION_ID).execute()
    try:
        list_of_instances = active_instances_dict["instances"]
        for instance in list_of_instances:
                request = appengine.apps().services().versions().instances()\
                    .delete(servicesId=settings.SERVICE_ID,
                            appsId=settings.PROJECT_ID,
